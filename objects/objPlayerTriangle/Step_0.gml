...
if varLook = 0
{
	varSPRIdle = sprPlayerTriangle
	varSPRWalking = sprPlayerTriangle
	varSPRRunning = sprPlayerTriangle
	varSPRJumpUp = sprPlayerTriangle
	varSPRJumpDown = sprPlayerTriangle
...
if not varAttacking
{	
	if keyboard_check_pressed(vk_down) and not varSmashing
	{
		if not place_meeting(x + 0, y + 1, objBlock)
		{
			varVSpeed = 0
			varSmashing = true
			alarm_set(0, 25)
	...
	if place_meeting(x + 0, y + 4, objBlock) and varSmashing
	{
		{
			if not instance_exists(objParticle)
			{
				if not varParticleing
				{
					instance_create_layer(x, y + 12, "Player", objParticle)
					instance_create_layer(x, y + 12, "Player", objParticle)
					instance_create_layer(x, y + 12, "Player", objParticle)
					instance_create_layer(x, y + 12, "Player", objParticle)
					instance_create_layer(x, y + 12, "Player", objParticle)
			...
			if instance_exists(objParticle)
			{
			if objParticle.varExistence != 5
				{
